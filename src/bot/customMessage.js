let customSendMessage = async (bot, msg, text, inlineButtons = [], parse_mode = "HTML") => {
    try {
        let sentMessage = await bot.sendMessage(msg.chat.id, text, { parse_mode: 'HTML', reply_markup: JSON.stringify({ inline_keyboard: inlineButtons, parse_mode: parse_mode })});
        return sentMessage;
    } catch(e) {
        console.log(e);
        return;
